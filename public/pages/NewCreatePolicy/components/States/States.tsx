...
const ReadOnlyState = ({ state, isInitialState }: StateProps) => (
  <EuiAccordion
    style={{ padding: "15px" }}
    id={state.name}
    buttonContent={
  ...
  >
    <EuiFlexGroup direction="column">
      ...
      <EuiFlexItem>
        <EuiFlexGroup>
          {!state.actions.length && <EuiText>No actions.</EuiText>}
          {state.actions.map((action) => (
            <EuiFlexItem grow={false}>
              <EuiPanel>{uiActionFactory.getUIActionFromData(action).content()}</EuiPanel>
            </EuiFlexItem>
      ...
      <EuiFlexItem>
        <EuiFlexGroup>
          {!state.transitions.length && <EuiText>No transitions.</EuiText>}
          {state.transitions.map((transition) => (
            <EuiFlexItem grow={false}>
              <EuiPanel>{Transition.content(transition)}</EuiPanel>
            </EuiFlexItem>
...
const States = ({ onOpenFlyout, policy, onClickEditState, onClickDeleteState, isReadOnly }: StatesProps) => {
